...
class FresnsPostsResource extends BaseAdminResource
{
    public function toArray($request)
    {
        ...
        if ($uri == '/api/v1/post/follows') {
            ...
            if ($followType == 'hashtag') {
                ...
                $hashtag = [];
                $hashtag['huri'] = $hashTagInfo['slug'] ?? null;
                $hashtag['hname'] = $hashTagInfo['name'] ?? null;
                $hashtag['cover'] = $hashTagInfo['cover_file_url'] ?? null;
                $default = [
                    'followType' => $followType,
                    'pid' => $pid,
                    'title' => $title,
    ...
    public static function getContentView($content, $postId, $postType, $content_markdown = 0)
    {
        ...
        // Sticker
        preg_match_all("/\[.*?\]/", $content, $stickers);
        if ($stickers[0]) {
            foreach ($stickers[0] as $e) {
                $stickerName = str_replace('[', '', $e);
                $sticker = str_replace(']', '', $stickerName);
                $stickerInfo = FresnsStickers::where('code', $sticker)->where('is_enable', 1)->first();
                if ($stickerInfo) {
                    $url = $stickerInfo['image_file_url'];
                    $content = str_replace($e, "<img src='$url' class='fresns_content_sticker' />", $content);
                }
            }
        }

        // mention @
        preg_match_all("/@.*?\s/", $content, $user);
        if ($user[0]) {
            foreach ($user[0] as $m) {
                $username = trim(str_replace('@', '', $m));
                $trimName = trim($m);
                $userInfo = FresnsUsers::where('username', $username)->first();
                if ($userInfo) {
                    $jumpUrl = ApiConfigHelper::getConfigByItemKey(FsConfig::SITE_DOMAIN)."/u/$username";
                    $content = str_replace($m, "<a href='{$jumpUrl}' class='fresns_content_mention'>@{$userInfo['nickname']}</a> ", $content);
                }
            }
        }
    ...
    public function contentByType($id)
    {
        ...
        if (! $followType) {
            ...
            $followUserArr = DB::table(FresnsUserFollowsConfig::CFG_TABLE)->where('user_id', $uid)->where('follow_type', 1)->pluck('follow_id')->toArray();
            $postUserIdArr = FresnsPosts::whereIn('user_id', $followUserArr)->pluck('id')->toArray();
            if (in_array($id, $postUserIdArr)) {
                $followType = 'user';
            }
        }
