...
{
    // table_id
    public static function getLanguagesByTableId($table, $table_column, $table_id)
    {
        if (! $table_id) {
            return '';
        }
    ...
    // table_key
    public static function getLanguagesByTableKey($table, $table_column, $table_key)
    {
        if (! $table_key) {
            return '';
        }
    ...
    // Get all languages
    public static function getAllLanguages($table, $table_column, $table_id)
    {
        if (! $table_id) {
            return '';
        }
    ...
    public static function getLangTag()
    {
        ...
        if ($isControlApi == 1) {
            $accountId = Auth::id();
            $langTag = request()->input('lang', Cache::get('lang_tag_'.$accountId));
        } else {
            $langTag = ApiLanguageHelper::getLangTagByHeader();
        }
