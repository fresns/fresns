        ...
        foreach ($this->searchableFields as $searchField => $arr) {
            ...
            switch ($upperOp) {
                ...
                case'JSON_NUMBER':
                    ...
                    // App\Fresns\Api\Account::where('meta->skills', 'like', '%Vue%')->get()
        ...
        foreach ($this->joinSearchableFields as $joinItem) {
            ...
            $this->dbQuery->join($joinTable, function ($join) use ($mainTable, $joinTable, $joinCondArr) {
                foreach ($joinCondArr as $joinCond) {
                    $mainTableColumn = $joinCond['main_table_column'];
                    $joinTableColumn = $joinCond['join_table_column'];
                    $op = $joinCond['op'];
                    $join->on("{$mainTable}.{$mainTableColumn}", $op, "{$joinTable}.{$joinTableColumn}");
                }
            });
        }
    }
